function [R_Dis_R,R_Dis_C,C_Dis_R,C_Dis_C,R_suffix,C_suffix,A] = MainFunc_Draw_Line(A)
% 将语义分割鉴定之后的图片转为Label作为输入
% R_Dis_R
% R_Dis_C
% C_Dis_R
% C_Dis_C
...
for k = 1:N
    R_suffix{1,k}=strcat(num2str(R_Dis{k,1}),format);% 文件后缀
    C_suffix{1,k}=strcat(num2str(C_Dis{k,1}),format);% 文件后缀
end
for k = 1:N
